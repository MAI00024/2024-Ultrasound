...
parser.add_argument('--fold', type=int, required=True, help='Select fold number')
...
# Training
train_losses, train_accuracies, val_losses, val_accuracies, epochs, lrs = train_model(model=model, optimizer=optim, n_epochs=CFG.N_EPOCHS, criterion=criterion, patience=CFG.PATIENCE, scheduler=scheduler)
...
def plot_metrics(epochs, train_losses, val_losses, train_accuracies, val_accuracies):
    ...
    plt.plot(range(1, epochs + 1), val_losses, 'ro-', label='Validation loss')
...
def evaluate_and_print(model, loader, df, name):
    misclassified_info = eval_model(model, loader, df, name)
    for name, info in misclassified_info.items():
        print(f'Image Name: {name}, Prediction: {info["pred"]}, Label: {info["label"]}')
    print()

evaluate_and_print(model, train_loader, train_df, 'train')
evaluate_and_print(model, val_loader, val_df, 'val')
evaluate_and_print(model, test_loader, test_df, 'test')

# Load pretrained model and evaluate on test set
def load_and_evaluate(select_Fold_case, model, device, test_loader, test_df):
    model = Net()
    load_path = f'./img_cls_fold{select_Fold_case}.pth'
    model.load_state_dict(torch.load(load_path))
    model.to(device)
    evaluate_and_print(model, test_loader, test_df, 'test')

load_and_evaluate(CFG.select_Fold_case, model, device, test_loader, test_df)
