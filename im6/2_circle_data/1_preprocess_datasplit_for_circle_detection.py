...
def create_file_list_txt(folder_path, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        for file_name in file_list:
            ...
            # Remove './split' part from the file path
            file_path = file_path.replace('split/', '')
            
            f.write(file_path + '\n')

# Function to split the dataset using GroupKFold
def split_dataset(images_path, labels_path, output_path, n_splits=10, random_state=42):
    ...
    # Extract device IDs to assign each image to a device group
    device_ids = [os.path.basename(img).split('_')[0] for img in all_images]
    print(device_ids)

    # Setting up GroupKFold cross-validation based on device IDs
    ...
    for fold, (train_valid_idx, test_idx) in enumerate(kf.split(all_images, groups=device_ids)):
        ...
        yaml_data = {
            "names": ['region', 'circle'],  # Class names
...
# Main function
def main():
    split_dataset("Result_region_detect/Result_crop_images", "Result_region_detect/Result_circle_updated", "split_for_circle_detection", random_state=42)
    print("Done.")
