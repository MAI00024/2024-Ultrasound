...
# Set the folder path
folder_path = './Result_circle_detect/preds'
...
df = pd.DataFrame(list(count_dict.items()), columns=['name', 'im6_preds_count'])

# Create a new column based on the count
df['im6_preds'] = df['im6_preds_count'].apply(lambda x: 1 if x >= 4 else 0)

# Part 2: Load labels and compare performance

# Load label data
df_labels = pd.read_csv('data/im6_label_updated240122.csv')
...
# Convert 'im6_preds_count' and 'im6_preds' columns to int
result_df['im6_preds_count'] = result_df['im6_preds_count'].astype(int)
result_df['im6_preds'] = result_df['im6_preds'].astype(int)

# Part 4: Check cases where predictions are incorrect

print(result_df[result_df['im6_label'] != result_df['im6_preds']])
...
result_df.to_csv(os.path.join(result_folder, "Im6_result.csv"), index=False)

# Part 5: Evaluate and visualize performance

# Actual labels and predictions
y_true = result_df['im6_label']
y_preds = result_df['im6_preds']
...
image_dir = './Result_circle_detect/images'
label_dir = './Result_circle_detect/preds'
